...
export const TimelineTopBar: React.FC<TimelineTopBarProps> = ({
  ...
  return (
    <div className="relative bg-white border-b border-slate-200 h-12 sm:h-16">
      <div
        className={`h-full flex items-start pl-4 sm:pl-44 pt-2 sm:pt-3 overflow-x-auto ${
          showActions ? 'pr-20 sm:pr-52' : 'pr-4 sm:pr-14'
        }`}
      >
      <nav
        role="navigation"
          ...
          return (
            ...
            >
              <button
                ...
                className={`px-1 py-1.5 text-[13px] sm:text-[15px] font-medium transition-colors focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-slate-900 ${
                  isActive
                    ? 'text-slate-900'
      ...
      <div className="absolute top-1/2 right-3 sm:right-4 -translate-y-1/2 flex items-center gap-2 bg-white pl-3">
        ...
        {onClose && (
          <button
            ...
            className="w-10 h-10 rounded-full border border-slate-300 text-slate-600 hover:bg-slate-100 flex items-center justify-center"
          >
